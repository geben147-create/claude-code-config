...
## Plan First, Code Later (3단계 방법론 기본 적용)
- **모든 개발 작업에 3단계 방법론 자동 적용** (명령어 호출 불필요)
...
- 큰 프로젝트: /gsd:plan-phase, 중간: 3단계 방법론 자동, 작은거: /gsd:fast
- 예외: 한 줄 수정, 간단한 질문, 버그 핫픽스는 바로 진행 OK

## Verification Loop (검증 루프 — 가장 중요!)
- → 3단계 방법론 Execute 단계에 통합됨 (테스트 먼저 → 검증 → 반복)
...
- → 3단계 방법론 마일스톤 단위에 통합됨 (1 마일스톤 = 1 커밋, ~100줄, TDD)
...
## 3-Phase Methodology (3단계 방법론)
For any non-trivial project, follow this 3-phase approach:

### Phase 1: Tech Spec (기술 사양서) — `/spec`
...
- `/spec` — Create tech spec with model council
- `/plan` — Create implementation plan from tech spec
- `/execute` — Execute next uncompleted milestone

---

## AI 리서치 파이프라인 (AI Research Pipeline)
...
2. 자연어 지시: "웹에서 데이터 검색 후 결과 구조화해서 반환하는 스킬 만들어줘"
